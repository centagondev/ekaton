...
export default function HomePage() {
  ...
  // Build the display label for the online count
  const countLabel = (() => {
    if (loading) return null;               // show pulsing dots while fetching
    if (count === null) return "—";         // endpoint not live yet — show dash
    ...
    <main className="flex min-h-[calc(100dvh-4rem)] flex-col items-center justify-center bg-[#FBF9F5] px-4 py-12 text-center">
      <div className="mx-auto flex max-w-2xl flex-col items-center space-y-8">
        ...
        <button
          ...
          className="group relative flex cursor-pointer items-center justify-center space-x-3 border-2 border-black bg-[#FFD600] px-8 py-4 text-base font-extrabold tracking-wider text-black uppercase shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] transition-all duration-150 hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-x-[5px] active:translate-y-[5px] active:shadow-none"
        ...
        <div className="flex flex-col items-center justify-center space-y-0.5 border-2 border-black bg-white px-5 py-2.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
          <div className="flex items-center space-x-2">
            {/* Pulsing green dot — always visible */}
            <span className="relative flex h-2.5 w-2.5">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75" />
              <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-emerald-500" />
            </span>

            {/* Count or loading dots */}
            {loading ? (
      ...
      {/* Bounce keyframes injected inline — keeps this self-contained */}
      <style>{`
        @keyframes bounce {
